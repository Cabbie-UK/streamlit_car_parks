...
def plot_volume(ids):
    ...
    # Initialize an empty DataFrame to store aligned y values
    aligned_y = pd.DataFrame(index=x.index)

    for id in ids:
        # Filter data for the current id
        id_data = df[df['parts_id'] == id][['date', 'volume']]
        # Merge the data with x based on date to align
        merged_data = pd.merge(x, id_data, on='date', how='left')
        # Fill missing values with 0
        merged_data['volume'].fillna(0, inplace=True)
        # Store the aligned y values
        aligned_y[id] = merged_data['volume']

    # Plot aligned data
    for id in ids:
        ax.plot(x, aligned_y[id], label=id)    
